...
@register(
    ...
    description=(
        ...
        "und ueber den Praefix `ds.<tabelle>` lesbar.\n\n"
        "Datastore-Schema (Hash-zentriert seit Pipeline-Reform v2):\n"
        "- ds.agent_sources: eine Zeile pro sha256, Spalten: id, sha256, size_bytes, "
        "kind ('source'|'context'), status, first_seen_at, last_seen_at\n"
        "- ds.agent_source_locations: eine Zeile pro Ablageort, Spalten: id, source_id, "
        "rel_path, logical_path, origin, status, filename, folder, extension, "
        "sp_item_id, sp_web_url, external_meta (JSON)\n"
        "- ds.agent_source_metadata: externe Metadaten pro source_id (key/value/source_of_truth)\n"
        "- ds.agent_doc_markdown: Markdown pro source_id (file_id = source_id)\n"
        "- ds.agent_doc_unit_offsets: Seiten-Offsets pro source_id\n"
        "- ds.agent_pdf_inventory, ds.agent_search_*\n\n"
        "Beispiele:\n"
        "- 'SELECT * FROM ds.agent_sources WHERE status=\"active\"' — alle Inhalte\n"
        "- 'SELECT s.id, l.rel_path FROM ds.agent_sources s "
        "JOIN ds.agent_source_locations l ON l.source_id=s.id WHERE l.status=\"active\"' "
        "— Inhalt + repräsentativer Pfad\n"
        "- 'SELECT s.id, COUNT(l.id) FROM ds.agent_sources s "
        "JOIN ds.agent_source_locations l ON l.source_id=s.id "
        "WHERE l.status=\"active\" GROUP BY s.id HAVING COUNT(l.id)>1' — Duplikate strukturell\n\n"
        "WICHTIG: rel_path/filename/extension leben in agent_source_locations, NICHT in "
        "agent_sources. Eine alte 'agent_source_relations'-Tabelle gibt es nicht mehr — "
        "Duplikate sind als mehrere Locations pro source sichtbar.\n\n"
